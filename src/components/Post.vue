      ...
      <div id="post" :key="post.id">
        ...
        <section id="relations">
          ...
          <router-link
            ...
            v-for="post of post.relation_posts"
            :key="post.id"
            class="post"
          >
            <article>
        ...
        <section id="comments">
          ...
          <div class="comment" v-for="comment of this.post.comment_set" :key="comment.id">
            ...
            <p class="do-reply">
              <a @click="doReply(comment)">↑返信する</a>
            </p>
          </div>
          <p class="do-comment">
            <a @click="doComment">新しくコメントする</a>
          </p>
              ...
              <div class="modal-wrapper" @click.self="closeForm">
                <div class="modal-container">
                  <div class="modal-body">
                    ...
                    <button type="button" @click="send" :disabled="sending">送信</button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    getPost() {
      this.$http(`${process.env.VUE_APP_API_POSTS}${this.id}/`, {
        credentials: "include",
      })
        .then((response) => {
    ...
    highlightToc() {
      ...
      for (const heading of this.$refs.text.querySelectorAll("h2,h3")) {
        const headingPos =
          window.pageYOffset + heading.getBoundingClientRect().top;
